...
# Custom effort
def get_frequent_itemsets(c_itemsets: list, orders: pd.DataFrame, min_support: float = 0.1) -> list:
    f_itemsets = []
    for item in c_itemsets:
        if get_support(item, orders) >= min_support:
            f_itemsets.append(item)
    return f_itemsets
